...
describe "Functor" <| fun () ->
  ...
  describe "#Invoke" <| fun () ->
    ...
    describe "when list" <| fun () -> 
      it "maps a function over a list" <| fun () -> promise {
    ...
    describe "when option" <| fun () -> 
      it "maps a function over Some" <| fun () -> promise {
    ...
    describe "when result" <| fun () -> 
      it "maps a function over Ok" <| fun () -> promise {
        let actual = Functor.Invoke (fun x -> x * 2) (Ok 10 : Result<int, string>)
        Expect.equal actual (Ok 20)
      }

      it "returns Error unchanged when input is Error" <| fun () -> promise {
        let actual = Functor.Invoke (fun x -> x * 2) (Error "Error message" : Result<int, string>)
        Expect.equal actual (Error "Error message")
      }

    describe "when function" <| fun () -> 
      it "composes a function with the mapping" <| fun () -> promise {
