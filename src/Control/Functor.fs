...
type Functor =
  static member Map(source: 'R -> 'T, mapping: 'T -> 'U, [<OptionalArgument>] _mthd: Functor) =
    source >> mapping
  ...
  static member Map
    (source: Result<'T, 'E>, mapping: 'T -> 'U, [<OptionalArgument>] _mthd: Functor)
    =
    Result.map mapping source

  static member Map(source: list<'T>, mapping: 'T -> 'U, [<OptionalArgument>] _mthd: Functor) =
    List.map mapping source
