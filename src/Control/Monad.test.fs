module Himanoa.Fable.Effect.Test.Control.Monad
...
<| fun () ->
  ...
  <| fun () ->
    ...
    describe "when list"
    <| fun () ->
      it "binds a function over a list"
      <| fun () ->
        promise {
    ...
    <| fun () ->
      ...
      it "returns the first Error when input is Error"
      <| fun () ->
        promise {
...
<| fun () ->
  ...
  <| fun () ->
    it "satisfies left identity for list"
    <| fun () ->
      promise {
        let returnM = fun x -> [ x ]
        let f = fun x -> [ x * 2 ]
    ...
    it "satisfies left identity for option"
    <| fun () ->
      promise {
        let returnM = Some
    ...
    it "satisfies left identity for result"
    <| fun () ->
      promise {
        let returnM = Ok
        let f = fun x -> Ok(x * 2)
  ...
  <| fun () ->
    it "satisfies right identity for list"
    <| fun () ->
      promise {
        let m = [ 1; 2; 3 ]
        let returnM = fun x -> [ x ]
    ...
    it "satisfies right identity for option"
    <| fun () ->
      promise {
        let m = Some 10
        let returnM = Some
    ...
    it "satisfies right identity for result"
    <| fun () ->
      promise {
        let m = Ok 10
        let returnM = Ok
  ...
  <| fun () ->
    it "satisfies associativity for list"
    <| fun () ->
      promise {
        let m = [ 1; 2; 3 ]
        let f = fun x -> [ x * 2 ]
    ...
    it "satisfies associativity for option"
    <| fun () ->
      promise {
        let m = Some 10
        let f = fun x -> Some(x * 2)
    ...
    it "satisfies associativity for result"
    <| fun () ->
      promise {
        let m = Ok 10
        let f = fun x -> Ok(x * 2)
